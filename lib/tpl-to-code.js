...
//编译模版
Tpl.pt.parse = function (tpl, tableName, columns,camelCase) {
    var that = this,
        tplog = tpl;
    ...
    try {
        that.cache = tpl = new Function('tableName,columns,_escape_,camelCase', tpl);
        return tpl(tableName, columns, tool.escape,camelCase);
    } catch (e) {
        delete that.cache;
...
Tpl.pt.render = function (data, callback,camelCase) {
    ...
    Object.keys(data).forEach(function (key) {
        ...
        tpl[filename] = that.cache ? that.cache(key, columns, tool.escape,camelCase) : that.parse(that.tpl, key, columns,camelCase);
