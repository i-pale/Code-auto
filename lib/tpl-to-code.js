...
//编译模版
Tpl.pt.parse = function (tpl, tableName, columns) {
    var that = this,
        tplog = tpl;
    ...
    try {
        that.cache = tpl = new Function('tableName,columns,_escape_', tpl);
        return tpl(tableName, columns, tool.escape);
    } catch (e) {
        delete that.cache;
...
Tpl.pt.render = function (data, callback) {
    ...
    Object.keys(data).forEach(function (key) {
        ...
        tpl[key] = that.cache ? that.cache(key, columns, tool.escape) : that.parse(that.tpl, key, columns);
        tpl[key] = tpl[key].replace(/_.d_/g,'').replace(/._r._n[\s|]+_tpl_No_val/g,"")
